...
/**
 * Primal\Database\Query - Asynchronous, chainable MySQL Query Builder
 *
 * @package Primal
...
class Query {
	 ...
	 * If this is not set, Query attempts to use Connection::Link()
	 *
	 * @var array
	 * @access protected
	 */
	 ...
	 *
	 * @static
	 * @param string $target The name of the primary table to be searched against
	 * @param string $alias An alias for the table name to be used in the query
	 * @return Query A new Query object
	 */
	static function Create($target = null, $alias='') {
	 ...
	 * @param string $alias An alias for the table name to be used in the query
	 */
	function __construct($target = null, $alias='') {
		if ($target instanceof PDO) $this->setPDOLink($target);
		elseif ($target !== null) $this->from($target, $alias);
	}
	 ...
	 * If no link has been defined, function will attempt to retrieve the default link from Primal\Database\Connection.
	 * Throws an exception if unable to find a link.
	 *
	 * @return PDO
	 */
	function getPDOLink() {
		$pdo = !$this->pdolink && class_exists('\\Primal\\Database\\Connection') ? Connection::Link() : $this->pdolink;

		if (!$pdo) throw new QueryException("No PDO Link has been defined.");
	...
	function createParam($data) {
		$key = ':' . static::AlphaInteger(count($this->parameters));
...
*/	
	...
	function whereDateInRange($fieldname, $from=0, $to=0) {
		$param_from = $this->createParam(static::ParseDate($from, static::DATEPARSE_DATE));
		$param_to = $this->createParam(static::ParseDate($to, static::DATEPARSE_DATE));
	...
	function whereTimeInRange($fieldname, $from=0, $to=0) {
		$param_from = $this->createParam(static::ParseDate($from, static::DATEPARSE_TIME));
		$param_to = $this->createParam(static::ParseDate($to, static::DATEPARSE_TIME));
	...
	function whereDateTimeInRange($fieldname, $from=0, $to=0) {
		$param_from = $this->createParam(static::ParseDate($from, static::DATEPARSE_DATETIME));
		$param_to = $this->createParam(static::ParseDate($to, static::DATEPARSE_DATETIME));
	...
	private static function ParseDate($input, $return) {
		if ($input) {
	 ...
	 * @return Query The current Query object.
	 */
	function whereInList($fieldname, array $values) {
		...
		$this->where[] = "$fieldname IN (".implode(",", $values).")";	
...
*/	
	...
	function setDate($fieldname, $value) {
		$this->set[] = "$fieldname = " . $this->createParam(static::ParseDate($value, static::DATEPARSE_DATE));
		return $this;
	}
	
	function setTime($fieldname, $value) {
		$this->set[] = "$fieldname = " . $this->createParam(static::ParseDate($value, static::DATEPARSE_TIME));
		return $this;
	}
	
	function setDateTime($fieldname, $value) {
		$this->set[] = "$fieldname = " . $this->createParam(static::ParseDate($value, static::DATEPARSE_DATETIME));
		return $this;
	}
...
*/
	 ...
	 *
	 * @param boolean $debug Optional. If true, function will return a unified string using Connection::Unprepare. DO NOT USE IN REAL CODE
...
*/	
	 ...
	 * @author Jarvis Badgley
	 */
	function insertSelect($target, $column) {
		list($query, $data) = $this->buildSelect();
...
*/	
	...
	/**
	 * Converts an integer into a base 24 alpha-only value
	 *
	 * @param string $n 
	 * @return void
	 * @author Jarvis Badgley
	 */
	protected static function AlphaInteger($n) {
	    $r = '';
	    for ($i = 1; $n >= 0 && $i < 10; $i++) {
