...
class Record extends \ArrayObject {
	...
	function __construct($o=null, $pdolink=null) {
		...
		//set up our pdo link.  if one is provided, always use it. If not, verify that Primal\Database\Connection exists and then grab the default link.
		if ($pdolink !== null) $this->pdolink = $pdolink;
		elseif (!$this->pdolink && class_exists('\\Primal\\Database\\Connection')) $this->pdolink = Connection::Link();
		else throw new RecordException('Could not find Primal\\Database\\Connection, you must provide a PDO database link.');
	...
	protected function tablename() {
		if ($this->dbname) return "`{$this->dbname}`.`{$this->tablename}`";
	 ...
	 * @return array
	 */
	static function Index($colname) {
		$o = new static();
		$tablename = $o->tablename();
		$q = "SELECT LEFT(`$colname`, 1) AS `firstletter`, COUNT(*) AS `counter` FROM {$tablename} GROUP BY `firstletter` ORDER BY `$colname`";
		
		$qs = $o->pdolink->prepare($q);
		$qs->execute();

		return $qs->rowCount() ? $qs->fetchAll(PDO::FETCH_ASSOC) : array();
	}
	 ...
	 *
	 * Syntax: Primal\Database\Record::BuildDefaults('tablename');
